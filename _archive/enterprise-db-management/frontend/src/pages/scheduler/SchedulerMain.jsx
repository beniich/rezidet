...
import { initSchedulerSocket } from '../../services/api';

export default function SchedulerMain() {
    const navigate = useNavigate();
    const [showAssignmentModal, setShowAssignmentModal] = useState(false);
    const [lastUpdate, setLastUpdate] = useState(new Date().toISOString());
    const [activePersonnelCount, setActivePersonnelCount] = useState(12);

    useEffect(() => {
        // Connect to the backend socket
        const socket = initSchedulerSocket();

        socket.on("connect", () => {
            console.log("Connected to Scheduler WebSocket");
        });

        socket.on("scheduler:sync", (data) => {
            setLastUpdate(data.lastUpdate);
            if (data.activePersonnel) setActivePersonnelCount(data.activePersonnel);
        });

        socket.on("scheduler:update", (data) => {
            setLastUpdate(new Date().toISOString());
            // In a real app, we would update the lists here
        });
        ...
        <div className="bg-background-dark font-display text-slate-100 antialiased overflow-hidden h-screen flex flex-col">
            ...
            <header className="bg-slate-900 border-b border-slate-800 h-16 flex items-center justify-between px-6 shrink-0 z-30">
                <div className="flex items-center gap-4">
                    <button onClick={() => navigate('/')} className="hover:bg-slate-800 p-2 rounded-lg transition-colors text-slate-400" title="Back to Dashboard">
                ...
                <div className="flex items-center gap-4">
                    <div className="flex flex-col items-end mr-2">
                        ...
                        <span className="text-[10px] text-slate-500 uppercase">Last update: {getRelativeTime(lastUpdate)}</span>
                    </div>
                    ...
                    <div className="flex-1 relative overflow-auto custom-scrollbar">
                        ...
                        <div className="relative min-h-full gantt-line">
                            ...
                            {[
                                ...
                                <div
                                    key={idx}
                                    onClick={() => navigate('/scheduler/team/01')}
            ...
            <div className="fixed bottom-16 right-6 w-80 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl z-50 overflow-hidden transform transition-all ring-1 ring-white/5">
                ...
                <div className="p-4 space-y-3">
                    ...
                    <button
                        onClick={() => navigate('/scheduler/conflicts')}
